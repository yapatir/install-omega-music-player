...
function SongItem({
  ...
  return (
    <div className={`song-item ${isActive ? "song-item--active" : ""}`}>
      ...
      <span className="song-item__duration">
      ...
      <div className="song-item__menu-wrap">
        ...
        {menuOpen && (
          ...
          >
            ...
            {playlists.length === 0 && (
              <span className="song-item__dropdown-empty">
                {t("noPlaylists")}
              </span>
            )}
          </div>
...
function PlaylistCard({ playlist, onSelect, isSelected }) {
  ...
  return (
    <div
      className={`playlist-card ${isSelected ? "playlist-card--active" : ""}`}
      onClick={onSelect}
    >
  ...
  const handleOnlineSearch = async () => {
    ...
    try {
      const res = await fetch(
        `https://freemusicarchive.org/api/get/tracks.json?limit=20&page=1&search=${encodeURIComponent(searchQuery)}`,
      );
      ...
      try {
        ...
        const res = await fetch(
          `https://pixabay.com/api/videos/?key=${key}&q=${encodeURIComponent(searchQuery)}&per_page=10`,
        );
        setSearchResults([]);
      } catch {
        setSearchResults([]);
            ...
            {displaySongs.length > 0 && (
              <>
                ...
                <button
                  className="action-btn"
                  onClick={() => {
                    const shuffled = [...displaySongs].sort(
                      () => Math.random() - 0.5,
                    );
        ...
        <div className="main-page__search-bar">
          <div className="search-input-wrap">
            ...
            <input
              ...
              placeholder={
                onlineSearch
                  ? `${t("searchSongs")} (${t("onlineOnly")})`
                  : t("searchPlaceholder")
              }
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              onKeyDown={(e) =>
                onlineSearch && e.key === "Enter" && handleOnlineSearch()
              }
