...
export const PERMISSIONS = {
  LIBRARY_READ:    { id: 'library.read',    label: 'Read your music library',           icon: '📚', risk: 'low' },
  LIBRARY_WRITE:   { id: 'library.write',   label: 'Add/remove songs & playlists',      icon: '✏️', risk: 'medium' },
  PLAYER_READ:     { id: 'player.read',     label: 'See currently playing track',       icon: '▶️', risk: 'low' },
  PLAYER_CONTROL:  { id: 'player.control',  label: 'Control playback (play/pause/skip)', icon: '🎮', risk: 'medium' },
  STORAGE_READ:    { id: 'storage.read',    label: 'Read plugin settings/data',         icon: '💾', risk: 'low' },
  STORAGE_WRITE:   { id: 'storage.write',   label: 'Save plugin settings/data',         icon: '💾', risk: 'low' },
  NETWORK:         { id: 'network',         label: 'Make network requests (fetch)',      icon: '🌐', risk: 'medium' },
  UI_PAGE:         { id: 'ui.page',         label: 'Add a page to the navigation',      icon: '🖥️', risk: 'low' },
  UI_VISUALIZER:   { id: 'ui.visualizer',   label: 'Render a custom visualizer',        icon: '🎨', risk: 'low' },
  METADATA:        { id: 'metadata',        label: 'Read/write song metadata & tags',   icon: '🏷️', risk: 'medium' },
};
...
export function buildPluginAPI(manifest, appContext, pluginStorage) {
  ...
  const guard = (perm, fn) => {
    if (!perms.has(perm)) {
      return () => { throw new Error(`Permission denied: ${perm}`); };
    }
    return fn;
  };
  ...
  return {
    // ── Library ──
    library: {
      getSongs: guard('library.read', () => appContext.library.map(s => ({ ...s }))),
      getPlaylists: guard('library.read', () => appContext.playlists.map(p => ({ ...p }))),
      getFavorites: guard('library.read', () => appContext.favorites.map(s => ({ ...s }))),
      addSongs: guard('library.write', (songs) => appContext.addToLibrary(songs)),
      createPlaylist: guard('library.write', (name) => appContext.createPlaylist(name)),
      addToPlaylist: guard('library.write', (plId, song) => appContext.addToPlaylist(plId, song)),
    },

    // ── Player ──
    player: {
      getCurrentSong: guard('player.read', () => appContext.currentSong ? { ...appContext.currentSong } : null),
      isPlaying: guard('player.read', () => appContext.isPlaying),
      getVolume: guard('player.read', () => appContext.volume),
      play: guard('player.control', (song, queue) => appContext.playSong(song, queue)),
      pause: guard('player.control', () => appContext.setIsPlaying(false)),
      resume: guard('player.control', () => appContext.setIsPlaying(true)),
      setVolume: guard('player.control', (v) => appContext.setVolume(Math.max(0, Math.min(1, v)))),
    },
    ...
    ui: {
      ...
      toast: (message, type = 'info') => {
        window.dispatchEvent(new CustomEvent('omega:toast', { detail: { message, type } }));
      },
    },
    ...
    metadata: {
      ...
      setTag: guard('metadata', async (songId, tag, value) => {
        ...
        const songs = appContext.library.map(s =>
          s.id === songId ? { ...s, tags: { ...(s.tags || {}), [tag]: value } } : s
        );
...
export function executePlugin(code, api) {
  // Use Function constructor as lightweight sandbox
  // Plugin gets 'omega' as its only global, no access to window/document directly
  const sandboxedCode = `
    "use strict";
  ...
  try {
    ...
    const factory = new Function('omega', sandboxedCode);
    return factory(api);
  } catch (err) {
