...
const OmegaIcon = ({ size = 28 }) => (
  <svg width={size} height={size} viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
    <circle cx="14" cy="13" r="9" stroke="currentColor" strokeWidth="2.2" fill="none"/>
    <path d="M10 22 L7 26 M18 22 L21 26" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round"/>
    <path d="M10 26 L18 26" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round"/>
...
export default function Navigation() {
  const { t, currentPage, setCurrentPage, navOpen, setNavOpen, setExitModalOpen } = useApp();
  ...
  return (
    <nav className={`nav ${navOpen ? 'nav--open' : 'nav--closed'}`}>
      <div className="nav__header">
        ...
        <button className="nav__toggle" onClick={() => setNavOpen(!navOpen)} title="Toggle navigation">
      ...
      <div className="nav__items">
        {navItems.map(item => (
          <button
            ...
            title={!navOpen ? t(item.labelKey) : undefined}
          >
            <span className="nav__item-icon">{item.icon}</span>
            {navOpen && <span className="nav__item-label">{t(item.labelKey)}</span>}
            {currentPage === item.id && <span className="nav__item-indicator" />}
          </button>
        ))}
      </div>
